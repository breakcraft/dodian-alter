        ...
        when(options("How am I doing so far?", "Who are you?", "Where are you?", "Nothing really.")) {
            FIRST_OPTION -> {
                ...
                if(player.getSlayerAssignment() == null) {
                    chatNpc("You need something new to hunt. Come and see me<br>>when you can and I'll give you a new task.", npc = master.id)
                } else {
                    chatNpc("You're currently assigned to kill ${player.getSlayerAssignment()!!.identifier.lowercase()}; only ${player.attr[SLAYER_AMOUNT]}<br>>more to go.", npc = master.id)
                }
            }
            SECOND_OPTION -> {
            ...
            THIRD_OPTION -> {
                ...
                chatNpc("You'll find me in Taverley. I'll be here when you need a<br>>new task.", npc = master.id)
            }
            FOURTH_OPTION -> {
